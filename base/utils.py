...
def get_currency_codes(api_key):
    response = requests.get(f"https://openexchangerates.org/api/currencies.json?app_id={api_key}")
...
def create_expense_dict(expenses, expression):
    ...
    aggregated_expenses = expenses.annotate(adjusted_amount=expression).values('category__name').annotate(total_amount=Sum('adjusted_amount'))
...
def convert_expenses_currency(expenses, prev_currency, pref_currency, exchange_rates):
    for exp in expenses:
        converted_amount = round(convert_currency(float(exp.amount), prev_currency, pref_currency, exchange_rates), 1)
...
def calculate_budget_monitoring_data(user_budget, user):
    budgets = user_budget.annotate(
        total_expenses=Sum(
            ExpressionWrapper(F('category__expense__amount') * F('category__expense__recurrence_interval'), output_field=FloatField()),
            filter=Q(category__expense__user=user),
            output_field=FloatField(),
        )
    )
    budget_dict = {budget.category.name: (budget.amount, budget.total_expenses) for budget in budgets}
...
def calculate_top_spender_data(expenses):
    expense_category_expression = ExpressionWrapper(F('category__expense__amount') * F('category__expense__recurrence_interval'), output_field=FloatField())
    category_amount_dict = create_expense_dict(expenses=expenses, expression=expense_category_expression)
    sorted_expenses_dict = dict(sorted(category_amount_dict.items(), key=lambda x: x[1], reverse=True))
...
def calculate_monthly_expenses_data(expenses): 
    ...
    while start_date <= current_date:
        ...
        curr_month_expenses = expenses.filter(
            Q(start_date__lte=end_date, end_date__gte=start_date) |  # Intersection with start_date and end_date
            Q(start_date__lte=end_date, end_date__isnull=True) |  # Start_date in the month, end_date is None (ongoing expense)
            Q(start_date__isnull=True, end_date__gte=start_date)  # End_date in the month, start_date is None (ongoing expense)
        ).aggregate(total=Sum('amount'))

        monthly_expenses_dict[start_date.strftime('%B %y')] = curr_month_expenses['total']
